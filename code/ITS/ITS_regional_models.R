...
#-------------------------------------------------------------------------------
# BINOMIAL ITS FUNCTION FOR ONE GROUP
#-------------------------------------------------------------------------------
...
# level change at table path provided. CSV of estimated recovery ORs. Recovery
# not calculated as post-interruption capped at 19 December 2020.
#-------------------------------------------------------------------------------
binomial_its_function_group <- function(outcomes_vec = outcomes,
		...
		plot_outcome <- function(outcome) {
			...
			# calculate deviance adjustment paramete
		...
		main_plot_data <- main_plot_data %>%
			...
			mutate_at(.vars = c("predicted_vals", "lci", "uci", "probline_noLdn", 
			                    "uci_noLdn", "lci_noLdn", "probline_0", "lci0", 
			                    "uci0"), 
								~. * 100) %>%
		...
		# alter name to mention if Christmas was included
		if (remove_xmas) {
		...
		forest_plot_df$outcome_name <- factor(forest_plot_df$outcome_name, 
					...
					write.csv(forest_plot_df, file = here::here(table_path, 
					   paste0("its_main_ORs_",start_lockdown,"_",lockdown_adjustment_period_wks,  
					          "_", pastename_year_cut_data,"_", group, ".csv")))
		
		forest_plot_df <- forest_plot_df %>%
			mutate(dummy_facet = "A")
		
		forest_plot_reduction <- forest_plot(forest_plot_df, 
		                                     forest_plot_df$dummy_facet, 
		                                     "C: Reduction") +
...
#-------------------------------------------------------------------------------
# BINOMIAL REGIONAL MODEL TO GET INTERACTION BETWEEN TIER GROUP AND RESTRICTIONS
#-------------------------------------------------------------------------------
# this ouputs the model estimates for lockdown, tier group, and the interaction
# between the two as a CSV ONLY. This model is a level change only ITS.
# inputs the same as for other models.
#-------------------------------------------------------------------------------
binomial_its_function_regional <- function(outcomes_vec = outcomes,
  ...
  plot_outcome <- function(outcome) {
    ...
    # Binomial regional model
    # Change in level 
    ldn_centre <- df_outcome$time[min(which(df_outcome$lockdown == 1))]
    
    # fit model, calculate lagged residuals to fit in final model
    binom_model1 <- glm(as.matrix(cbind(numOutcome, numEligible)) ~ lockdown 
                        + group + group:lockdown + as.factor(months) , 
                        family=binomial, data = 
                          filter(df_outcome, !is.na(lockdown)))
    ci.exp(binom_model1)
    ...
    # fit model again with lagged residuals 
    binom_model2 <- glm(as.matrix(cbind(numOutcome, numEligible)) ~ lockdown 
                        + group + group:lockdown + as.factor(months) 
                        + binom_lagres, family=binomial, 
                        data = filter(model_data, !is.na(lockdown)))
    ...
    # Get ORs for effect of lockdown (change in level only)
    parameter_estimates <- as.data.frame(ci.exp(binom_model2))
    ...
    # mutate the columns for better printing
    vals_to_print <- merge(vals_to_print, p_val_estimates, by=0, all=TRUE, 
                           row.names=FALSE)
    colnames(vals_to_print) <- c("term", "exp(Est.)", "2.5%", "97.5%", 
                                 "outcome", "p-value")
    
    # select only terms wanted
  ...
  # changes the names of outcomes to full names
  estimates_data $outcome_name <- factor( estimates_data $outcome_name, 
                                          levels = outcome_of_interest_namematch$outcome_name[plot_order])
